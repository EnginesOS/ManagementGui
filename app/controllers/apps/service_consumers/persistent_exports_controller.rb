    ...
    class PersistentExportsController < ApplicationController
      ...
      def show
        ...
        if data_file.present?
          send_data data_file,
            filename: "engines_data_export_#{@app.name}__"\
              "#{params[:publisher_type_path].gsub '/', '_'}_"\
              "#{params[:service_handle]}__#{Time.now.utc}.gzip"
        else
          redirect_to cloud_path(cloud_id: @app.cloud.id),
            alert: "Failed to export data from "\
              "#{params[:service_handle]} for #{@app.name}."
        end
      rescue EnginesError => e
        raise EnginesError.new "Failed to export data from "\
          "#{params[:service_handle]} for #{@app.name}.\n\n#{e}"
      end
